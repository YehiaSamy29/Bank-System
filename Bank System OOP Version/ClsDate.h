...
public:
	...
	static int GetCurrentSystemHour() {
		time_t t = time(0);
		tm* now = localtime(&t);
		return now->tm_hour;
	}
	static int GetCurrentSystemMinute() {
		time_t t = time(0);
		tm* now = localtime(&t);
		return now->tm_min;
	}
	static int GetCurrentSystemSecond() {
		time_t t = time(0);
		tm* now = localtime(&t);
		return now->tm_sec;
	}
	static string GetCurrentDateTime() {
		string Date = "";
		Date += to_string(GetCurrentSystemHour()) + ":";
		Date += to_string(GetCurrentSystemMinute()) + ":";
		Date += to_string(GetCurrentSystemSecond());
		return Date;
	}
